    ...
    with tqdm(desc="Fetching topics") as pbar:
        while cursor:
            params = {
                "per-page": per_page,
                "cursor": cursor,
                "mailto": "chinardankhara@gmail.com"  # Replace with your email for API usage
            }
...
def validate_and_clean_topic(topic: Dict[str, Any]) -> Dict[str, Any]:
    ...
    # Extract hierarchical relationship if available
    # Adjust based on OpenAlex's actual fields for hierarchy
    parent_id = topic.get("parent_id")  # Assuming 'parent_id' field exists
    if not parent_id:
        parent_id = None  # Handle cases where there is no parent
    ...
    return {
        ...
        "parent_id": parent_id  # Include parent_id for hierarchy
    }
...
def process_hierarchy(topics: List[Dict[str, Any]]) -> List[Dict[str, str]]:
    """
    Extracts hierarchical relationships from topics.
    Returns a list of dictionaries with ancestor_id and child_id.
    """
    hierarchy = []
    for topic in topics:
        parent_id = topic.get("parent_id")  # Adjust based on OpenAlex's actual field
        if parent_id:
            hierarchy.append({"ancestor_id": parent_id, "child_id": topic["id"]})
    return hierarchy
...
def save_hierarchy_data(hierarchy: List[Dict[str, str]], data_dir: Path) -> None:
    """Save hierarchy data to a JSON file."""
    with open(data_dir / "topic_hierarchy.json", "w", encoding="utf-8") as f:
...
def main() -> None:
    ...
    # Extract and save hierarchy
    print("Processing hierarchical relationships...")
    hierarchy = process_hierarchy(cleaned_topics)
    save_hierarchy_data(hierarchy, data_dir)
