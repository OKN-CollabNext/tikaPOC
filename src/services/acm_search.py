...
class ACMSearcher:
    ...
    def search(self, query: str, max_results: int = 5) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            logger.info(f"ACM Digital Library API search successful for query: {query}")
        except requests.RequestException as e:
            logger.error(f"ACM Digital Library API error: {e}")
            return []
        ...
        try:
            ...
            logger.error(f"Error parsing ACM Digital Library API response: {e}")
            return []
