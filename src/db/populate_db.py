    ...
    with get_db_connection() as conn:
        with conn.cursor() as cur:
            ...
            for keyword_batch in tqdm(batch_iterator(keyword_list, batch_size),
                ...
                # Get embeddings for the batch using the embedder
            ...
            hierarchy_relations = [
                (relation["ancestor_id"], relation["child_id"])
                for topic in topics
                if "parent_id" in topic and topic["parent_id"]
                for relation in [{"ancestor_id": topic["parent_id"], "child_id": topic["id"]}]
            ]

            if hierarchy_relations:
                print("Inserting hierarchical relationships...")
                for batch in batch_iterator(hierarchy_relations, batch_size):
                    execute_values(
                        cur,
                        """
                        INSERT INTO topic_hierarchy (ancestor_id, child_id)
                        VALUES %s
                        ON CONFLICT DO NOTHING
                        """,
                        batch
                ...
                print("No hierarchical relationships to insert.")
...
def sanity_check_data() -> None:
    ...
    with get_db_connection() as conn:
        with conn.cursor() as cur:
            ...
            cur.execute("SELECT COUNT(*) FROM topic_hierarchy")
            hierarchy_count = cur.fetchone()[0]
            print(f"Total hierarchical relationships: {hierarchy_count}")
